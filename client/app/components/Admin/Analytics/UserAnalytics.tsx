...
import {} from 'next13-progressbar';
type Props = {
  isDashboard?: boolean;
};
    ...
    <>
      {isLoading ? (
        ...
        <div className='mb-16 mt-28'>
          <h1
            onClick={() => {}}
            className={`${styles.title} ml-8 ${
              isDashboard && '!text-[20px]'
            } !text-start !p-0 lg:ml-11 mb-4`}
          >
            Users Analytics
          </h1>
          <div className={`${!isDashboard ? '' : ''}`}>
            <div className={`${isDashboard ? 'lg:!ml-8 mb-5' : ''}`}>
              {!isDashboard && (
                <p className={`${styles.label} px-8`}>
