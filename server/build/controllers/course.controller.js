...
exports.editCourse = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        //   { new: true }
        // );
        const course = await course_model_1.default.findOneAndUpdate({ _id: courseId }, { $set: { ...data } }, { new: true });
...
exports.getSingleCourse = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        await section_model_1.default.find({});
        await video_model_1.default.find({});
        const courseId = req.params.id;
        ...
        if (isCacheExist) {
            ...
            const course = await course_model_1.default.findOne({
                ...
                populate: {
                    ...
                    // options: { sort: { 'questions.createdAt': -1 } },
                },
            });
            // .select(
...
exports.getAllCourses = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        const courses = await course_model_1.default.find({ published: true }).select('-courseData.videoUrl -courseData.suggestion -courseData.questions -courseData.links');
        res.status(200).json({
            success: true,
            courses,
        });
...
exports.getCourseByUser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        // console.log(course, 'courseis');
        // console.log(course, 'course');
        const content = course?.sections;
        // console.log(course, 'conten');
        res.status(200).json({
            success: true,
            content,
        });
...
exports.addQuestion = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        // }
        // const videos = course?.sections.map((section) => section.videos).flat();
        // console.log(videos, 'videos');
        const video = await video_model_1.default.findById(contentId);
        // const video: any = videos?.find((item: any) =>
        ...
        await notification_Model_1.default.create({
            ...
            message: `You have a new question in ${video.title}`,
        });
...
exports.addAnwser = (0, catchAsyncErrors_1.CatchAsyncError)(async (req, res, next) => {
    try {
        ...
        const question = video?.questions?.find((item) => item._id.equals(questionId));
        if (!question) {
        ...
        if (req.user?._id === question.user._id) {
            ...
            await notification_Model_1.default.create({
                ...
                message: `You have a new question reply in ${video.title}`,
            });
        }
        else {
            const data = {
